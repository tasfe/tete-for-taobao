    ...
    public class TradeSuccess
    {
        ...
        { 
            ...
            if (shop.IsKefu == "1")
            {
                //更新该订单的评价为待审核状态
                TradeData dbTrade = new TradeData();
                dbTrade.UpdateTradeKefuById(trade);
                return;
            }
            ...
            {
                //如果符合赠送条件调用赠送接口
                string couponId = api.SendCoupon(trade.BuyNick, shop.CouponID);
                ...
                if (shop.MsgIsCoupon == "1" && int.Parse(shop.MsgCount) > 0)
                {
                    ...
                    {
                        //发送短信
                        string msg = Message.GetMsg(shop.MsgCouponContent, shop.MsgShopName, TradeInfo.BuyNick, shop.MsgIsCoupon);
