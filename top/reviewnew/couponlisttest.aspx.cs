...
public partial class top_review_couponlist : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        MatchCollection match = new Regex(@"<coupon>([\s\S]*)</coupon>", RegexOptions.IgnoreCase).Matches(result);
        for (int i = 0; i < match.Count; i++)
        {
            string coupon_id = GetProperty("coupon_id", match[i].Groups[1].ToString());
            string denominations = GetProperty("denominations", match[i].Groups[1].ToString());
            string creat_time = GetProperty("creat_time", match[i].Groups[1].ToString());
            string end_time = GetProperty("end_time", match[i].Groups[1].ToString());
            string condition = GetProperty("condition", match[i].Groups[1].ToString());

            Response.Write(coupon_id + "--");
            Response.Write(denominations + "--");
            Response.Write(creat_time + "--");
            Response.Write(end_time + "--");
            Response.Write(condition + "--<br>");
        }
    }

    private string GetProperty(string key, string msg)
    {
        string coupon_id = new Regex(@"<" + key + ">([^<]*)</" + key + ">", RegexOptions.IgnoreCase).Match(msg).Groups[1].ToString();
