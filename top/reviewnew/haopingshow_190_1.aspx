<%@ Page Language="C#" AutoEventWireup="true" CodeFile="haopingshow_190_1.aspx.cs" Inherits="top_reviewnew_haopingshow_190_1" %>
...
<div style="position:absolute; width:100%; z-index:10;background-image: url(/medias/images/title_bar.png); margin:0px; line-height:30px; height:30px; font-size:12px; padding:0px 0px 0px 10px; font-weight:bold;"><a href="http://fuwu.taobao.com/serv/detail.htm?service_id=4545" target="_blank">好评有礼</a></div>
...
<asp:Repeater ID="rptTradeRate" runat="server">
    <ItemTemplate>
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="hp_list" id="rate109199846748230">
  <tr>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="top"><a href="http://item.taobao.com/item.htm?id=<%#Eval("itemid") %>" target="_blank">
            ...
            <div class="share_btn_box" rid="109199846748230"></div></td>
        </tr>
      </table>
      <div class="rate_box"><span class="rate_ico"></span><span class="vip_text">[好评]</span><a href="http://item.taobao.com/item.htm?id=<%#Eval("itemid") %>" target="_blank"><span class="rate_content"><%#Eval("content") %></span><span class="params" num_iid="4537379718" pic_url="http://img02.taobaocdn.com/bao/uploaded/i2/T1SOJxXXpiXXcCddjb_093429.jpg"></span></a></div></td>
  </tr>
  <tr>
    <td valign="bottom"><div class="buyer_box"><%#Eval("buynick") %><br />
      </div></td>
  </tr>
</table>
    </ItemTemplate>
...
<!--
<script type="text/javascript" src="/medias/js/jquery-1.5.2.min.js"></script>
<script>    var pages = parseInt('3'); var total_num = parseInt('26'); var seller_name = encodeURI('etanliuyang'); var page_no = Math.floor(Math.random() * pages) + 1; var width = '190'; var c_record = 0; var loop_index = 0; var up_switch; var is_first = true; if (pages > 1 && page_no == pages) { page_no = 1 }; function get_rates() { if (page_no > pages) page_no = 1; $.ajax({ url: '/front/rates/' + width + '/' + seller_name + '/' + page_no + '/', type: 'GET', dataType: 'html', timeout: 200000, success: function (datas) { if (is_first) { $('#hp_box').html(datas); is_first = false; } else { $('#hp_box').append(datas); } loop_index += 1; } }); page_no += 1; } get_rates(); var i = 4; function go_up() { i += 1; if (i == 10) { c_record += 1; if (c_record == 5) { c_record = 0 }; if ((loop_index < pages) && c_record == 0) { get_rates() }; var init_top = ($('#hp_box').offset().top); var temp_row = $('.hp_list').first(); $("#hp_box").animate({ "top": -temp_row.height() - 2 }, 5000, function () { $('#hp_box').append(temp_row.detach()).offset({ top: init_top }) }); i = 0; } } function timer() { up_switch = setInterval('go_up()', 1000); if (total_num < 3 && $("#hp_box").height() < 480) clearInterval(up_switch); } function mouse_switch() { $('#hp_box').mouseover(function () { clearInterval(up_switch); }).mouseout(function () { timer() }); } timer(); mouse_switch();</script>
