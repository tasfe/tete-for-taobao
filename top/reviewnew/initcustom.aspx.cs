...
public partial class top_crm_initcustom : System.Web.UI.Page
{
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        sql = "SELECT crmupdatedate FROM TCS_ShopConfig WHERE nick = '" + nick + "'";
        string lastdate = utils.ExecuteString(sql);

        //如果是第一次获取，则时间为当前的3个月前
        if (lastdate.Length == 0)
        {
            lastdate = DateTime.Now.AddMonths(-3).ToString("yyyy-MM-dd") + " 00:00:00";
        }
        ...
        //更新最后同步会员时间
        string sqlnew = "UPDATE TCS_ShopConfig SET crmupdatedate = GETDATE() WHERE nick = '" + nick + "'";
