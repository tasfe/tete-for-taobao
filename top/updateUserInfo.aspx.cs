...
public partial class top_updateUserInfo : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        string u = utils.NewRequest("u", utils.RequestType.QueryString);
        string t = utils.NewRequest("t", utils.RequestType.QueryString);

        if (u == "")
            return;

        string sql = "SELECT TOP 100 * FROM TopBlogAuto a INNER JOIN TopTaobaoShop s ON s.nick = a.nick AND s.isoverblog = 0 WHERE isauto = 1 AND keywords <> '' AND uids <> ''";
        DataTable dt = utils.ExecuteDataTable(sql);
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            CheckUser("0", dt.Rows[i]["nick"].ToString());
        }
    }
    ...
    private void CheckUser(string t, string u)
    {
        ...
        if (result.IndexOf("invali") != -1 || result.IndexOf("<appstore_subscribe_get_response></appstore_subscribe_get_response>") != -1)
        {
            //到期了
            if (t == "0")
            {
