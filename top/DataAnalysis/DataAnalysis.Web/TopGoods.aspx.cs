...
public partial class TopGoods : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            ViewState["count"] = taoGoodsService.GetTopGoodsCount(DataHelper.Encrypt(Request.Cookies["nick"].ToString()), darray[0], darray[1]);
            try
            {
    ...
    private void Bind(DateTime start, DateTime end, int totalCount, int recordCount)
    {
        ...
        IList<GoodsInfo> list = taoGoodsService.GetTopGoods(DataHelper.Encrypt(Request.Cookies["nick"].ToString()), start, end, page, 10);
