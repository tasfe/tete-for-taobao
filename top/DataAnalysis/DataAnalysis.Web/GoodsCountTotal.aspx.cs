...
public partial class GoodsCountTotal : BasePage
{
    ...
    private void ShowChart(DateTime start, DateTime end)
    {
        ...
        IList<TopSiteTotalInfo> list = new SiteTotalService().GetGoodsCountTotal(start.ToString("yyyyMMdd"), end.ToString("yyyyMMdd"), nick);
        SeriseText = "[{name:'商品数', data:[";
        string sucss = ",{name:'订单金额', data:[";
        string onesell = ",{name:'销售单价',data:[";
        ...
        for (DateTime i = start; i <= end; i = i.AddDays(1))
        {
            ...
            {
                SeriseText += mylist.Sum(o => o.GoodsCount) + ",";
                sucss += mylist.Sum(o => o.SiteOrderPay) + ",";
                onesell += Math.Round(mylist.Sum(o => o.SiteOrderPay) / mylist.Sum(o => o.SiteOrderPay), 2).ToString() + ",";
            }
        }
