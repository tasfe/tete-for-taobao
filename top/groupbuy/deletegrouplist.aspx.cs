...
public partial class top_groupbuy_deletegrouplist : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        ...
        string sqlNew = "SELECT TOP " + pageCount.ToString() + " * FROM (SELECT *,ROW_NUMBER() OVER (ORDER BY b.id DESC) AS rownumber FROM TopGroupBuy b WHERE b.nick = '" + taobaoNick + "' AND isdelete = 1) AS a WHERE a.rownumber > " + dataCount.ToString() + " ORDER BY id DESC";
        ...
        //分页数据初始化
        sqlNew = "SELECT COUNT(*) FROM TopGroupBuy WHERE nick = '" + taobaoNick + "' AND isdelete = 1";
        int totalCount = int.Parse(utils.ExecuteString(sqlNew));

        lbPage.Text = InitPageStr(totalCount, "deletegrouplist.aspx");
    }
