        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (i == 0)
            {
                ...
                if (templatehtmlUrl == "tpl/style3.html")
                {
                    ...
                    smailtempStr = smailtempStr.Replace("{leftprice}", (decimal.Parse(dt.Rows[i]["price"].ToString()) - decimal.Parse(dt.Rows[i]["proprice"].ToString())).ToString().Split('.')[0]);
                    smailtempStr = smailtempStr.Replace("{rightprice}", (decimal.Parse(dt.Rows[i]["price"].ToString()) - decimal.Parse(dt.Rows[i]["proprice"].ToString())).ToString().Split('.')[1]);
                    smailtempStr = smailtempStr.Replace("{newprice}", dt.Rows[i]["proprice"].ToString());
                    smailtempStr = smailtempStr.Replace("{buycount}", dt.Rows[i]["rcount"].ToString());
            ...
            else
            {
                ...
                if (dt.Rows[i]["templetID"].ToString() == "2" || dt.Rows[i]["templetID"].ToString() == "3")
                {
                    ...
                    smailtempStr = smailtempStr.Replace("{name}", dt.Rows[i]["name"].ToString());
                    smailtempStr = smailtempStr.Replace("{oldprice}", dt.Rows[i]["price"].ToString());
                    smailtempStr = smailtempStr.Replace("{zhekou}", Math.Round((decimal.Parse(dt.Rows[i]["price"].ToString()) - decimal.Parse(dt.Rows[i]["zhekou"].ToString())) / decimal.Parse(dt.Rows[i]["price"].ToString()) * 10, 1).ToString());
                    smailtempStr = smailtempStr.Replace("{leftprice}", (decimal.Parse(dt.Rows[i]["price"].ToString()) - decimal.Parse(dt.Rows[i]["proprice"].ToString())).ToString().Split('.')[0]);
                    smailtempStr = smailtempStr.Replace("{rightprice}", (decimal.Parse(dt.Rows[i]["price"].ToString()) - decimal.Parse(dt.Rows[i]["proprice"].ToString())).ToString().Split('.')[1]);
                    smailtempStr = smailtempStr.Replace("{newprice}", dt.Rows[i]["proprice"].ToString());
                    smailtempStr = smailtempStr.Replace("{buycount}", dt.Rows[i]["rcount"].ToString());
