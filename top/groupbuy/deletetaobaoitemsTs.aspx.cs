...
public partial class top_groupbuy_deletetaobaoitemsTs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        tSaid = utils.NewRequest("tSaid", Common.utils.RequestType.QueryString);
                ...
                if (dtWrite != null)
                {
                    ...
                    for (int j = 0; j < dtWrite.Rows.Count; j++)
                    {
                        ...
                        try
                        {
                            ...
                            if (!Regex.IsMatch(product.Desc, @"<div>[\s]*<a name=""tetesoft-area-start-" + tSaid + @""">[\s]*</a>[\s]*</div>[\s]*([\s\S]*)<div>[\s]*<a name=""tetesoft-area-end-" + tSaid + @""">[\s]*</a>[\s]*</div>"))
                            {
                                //如果没有代码
                                ...
                                {
                                    newContent = Regex.Replace(product.Desc, @"<div>[\s]*<a name=""tetesoft-area-start-" + tSaid + @""">[\s]*</a>[\s]*</div>[\s]*([\s\S]*)<div>[\s]*<a name=""tetesoft-area-end-" + tSaid + @""">[\s]*</a>[\s]*</div>", @"");
