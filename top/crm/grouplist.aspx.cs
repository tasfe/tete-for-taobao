...
public partial class top_crm_grouplist : System.Web.UI.Page
{
    ...
    private void UpdateData()
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            {
                //获取符合条件的会员并更新会员分组ID
                sql = "UPDATE TCS_Customer SET groupguid = '" + dt.Rows[i]["guid"].ToString() + "' WHERE nick = '" + nick + "' AND cast(tradeamount as decimal(18,2)) >= " + dt.Rows[i]["price"].ToString() + "";
                Response.Write(sql);
            ...
            {
                //获取符合条件的会员并更新会员分组ID
                sql = "UPDATE TCS_Customer SET groupguid = '" + dt.Rows[i]["guid"].ToString() + "' WHERE nick = '" + nick + "' AND cast(tradeamount as decimal(18,2)) >= " + dt.Rows[i]["price"].ToString() + " AND cast(tradeamount as decimal(18,2)) < " + dt.Rows[i + 1]["price"].ToString() + "";
                Response.Write(sql);
                utils.ExecuteNonQuery(sql);
            }
