...
public partial class top_crm_initcustombycsv : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 1; i < arr.Length-1; i++)
        {
            ...
            string msg = "【77的店】双11活动提前开始，fancal，欧舒丹新品登场 优惠劵满额赠礼http://77dedian.taobao.com";
            //判断该顾客信息是否录入过
            sql = "SELECT COUNT(*) FROM tcs_msgsend WHERE nick = '" + nick + "' AND buynick = '" + buynick + "' AND DATEDIFF(D, adddate, GETDATE()) = 0 AND typ = 'act'";
            //Response.Write(sql + "<br>");
            string count = utils.ExecuteString(sql);
            if (count == "0")
            {
                string result = SendGuodu(mobile, msg);
        ...
        Response.Write("<script>alert('发送成功，共发送" + index.ToString() + "名会员！');window.location.href='customlist.aspx';</script>");
        Response.End();
    }
