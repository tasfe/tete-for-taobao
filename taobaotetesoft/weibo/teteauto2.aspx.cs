...
public partial class weibo_teteauto1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //每小时最多一键收听一次
        sql = "SELECT uid FROM TopMicroBlogAccount WHERE typ = 'qq' AND score > 150 AND uid <> '' AND id > 672";
    ...
    private void autoListen(string uid)
    {
        ...
        //每小时最多一键收听一次
        sql = "SELECT TOP 60 * FROM TopMicroBlogAccount WHERE typ = 'qq' AND score > 0 AND uid <> '' AND id < 672 ORDER BY NEWID()";
        DataTable dt = utils.ExecuteDataTable(sql);
        for (int i = 0; i < dt.Rows.Count; i++)
        {
