    ...
    private void ShowSpecialListInfo()
    {
        ...
        else
        {
            ...
            if (cid.Length == 0)
            {
                if (uid == "taobao1")
                {
                    sql = "SELECT TOP " + pageCount.ToString() + " * FROM (SELECT *,ROW_NUMBER() OVER (ORDER BY NEWID) AS rownumber FROM TeteShopItem WHERE nick = '" + uid + "' AND width <> height AND isnew = 1) AS a WHERE a.rownumber > " + dataCount.ToString() + " ORDER BY NEWID";
                }
                else
                {
            ...
            else
            {
                if (uid == "taobao1")
                {
                    sql = "SELECT TOP " + pageCount.ToString() + " * FROM (SELECT *,ROW_NUMBER() OVER (ORDER BY NEWID) AS rownumber FROM TeteShopItem WHERE nick = '" + uid + "' AND isnew = 1 AND width <> height AND " + con + ") AS a WHERE a.rownumber > " + dataCount.ToString() + " ORDER BY NEWID";
                }
                else
                {
