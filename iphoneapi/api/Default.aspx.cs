    ...
    private void ShowSpecialListInfo()
    {
        ...
        if (dt.Rows.Count != 0)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                str += "{\"itemid\":\"" + dt.Rows[i]["itemid"].ToString() + "\",\"pic_url\":\"" + dt.Rows[i]["picurl"].ToString() + "_120x120.jpg\",\"name\":\"" + dt.Rows[i]["itemname"].ToString() + "\",\"detail_url\":\"" + dt.Rows[i]["linkurl"].ToString() + "\"}";
            }
