...
public partial class api_Default : System.Web.UI.Page
{
    ...
    private void TicketOrderPost()
    {
        ...
        //返回车票价格阶梯
        string ticketList = Regex.Match(result, @"var passengerJson = ([[][^[]]]*][]]);").Groups[1].ToString();

        //返回联系人列表
        string userList = Regex.Match(result, @"var limitBuySeatTicketDTO = ([^;]*);").Groups[1].ToString();
