...
public partial class api_Default : System.Web.UI.Page
{
    ...
    private void OrderPayPost()
    {
        ...
        if (result.IndexOf("该车次在互联网已停止办理业务") == -1)
        {
            ...
            resStr += "|网银支付（银联）,网银支付（银联）,https://epay.12306.cn/pay/webBusiness," + utils.PostData(param1) + ",,alert(1);setTimeout('direct()', 1000); function direct(){if(document.getElementById('CSPayTab')){window.location.href=document.getElementById('CSPayTab').getAttribute('href');}}";
